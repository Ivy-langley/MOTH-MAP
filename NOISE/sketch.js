...
function draw() {
  ...
  for (let i = 0; i < points.length; i++) {
    ...
    for (let j = i + 1; j < points.length; j++) {
      ...
      let avgR = (r1 + r2) / 2;
      let noiseValue = noise((r1 + r2) * 0.01, (r1 + r2) * 0.01, noiseOffset);
      let strokeWeightValue = map(noiseValue * avgR, 0, 300, 0.5, );

      strokeWeight(strokeWeightValue);
      stroke(noiseValue * 255); 

      line(x1, y1, x2, y2); // Connect points
    }

     // Map radius to greyscale value for the points
