...
function setup() {
  ...
  noLoop(); // Draw only once for performance (can be removed for animations)
...
function draw() {
  for (let i = 0; i < points.length; i++) {
    ...
    // Call sprayPaint function for each point
    sprayPaint(x, y, r);
  }
}

// Spray paint effect
function sprayPaint(x, y, radius) {
  stroke(0, 0, 0, 100); // White spray, semi-transparent
  strokeWeight(1); // Thin spray points

  const rSquared = radius * radius; // Pre-compute radius squared
  const sprayDensity = map(radius, 0, 150, 50, 600); // Adjust density based on radius
  const lerps = 20; // Number of steps for a dense effect

  for (let i = 0; i < lerps; i++) {
    // Draw random points within the radius
    for (let j = 0; j < sprayDensity; j++) {
      const randX = random(-radius, radius);
      const randY = random(-1, 1) * sqrt(rSquared - randX * randX);

      point(x + randX, y + randY); // Draw point around center (x, y)
    }
  }
}
